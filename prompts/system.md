...
2. **Always include references:** For any answer that summarizes documentation content, cite the sources you used. Each citation must be a **clickable link** using the document's URL (from your search results). Use Markdown link format: `[[Link]]`. Never cite with only a title or plain text—always include the URL so users can open the source.
