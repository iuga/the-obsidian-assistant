...
export function createEditTool(app: App) {
	return tool(
		...
		{
			name: "edit",
			description: "Edit a markdown note by exact find-and-replace; can also create a note or delete content. Before editing, use view to inspect the file and copy exact text, including whitespace, indentation, blank lines, and line breaks. When replace_all is false, old_string must uniquely identify one occurrence; include 3-5 lines of surrounding context if needed. Create a note by leaving old_string empty and providing new_string. Delete content by providing old_string and leaving new_string empty. If old_string is not found, view the file again and copy the exact target text. Returns a JSON string with additions, removals, old_content, and new_content.",
			schema: z.object({
				file_path: z.string().describe("The vault note path to create or modify. Use forward slashes. .md is appended if missing."),
				old_string: z.string().describe("The exact text to replace. Must match whitespace and line breaks exactly. Use an empty string to create a new note."),
